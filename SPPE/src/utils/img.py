...
def transformBoxInvert_batch(pt, ul, br, inpH, inpW, resH, resW):
    ...
    _pt[:, :, 0] = _pt[:, :, 0] - ((lenW[:, np.newaxis].repeat(1, 4) - 1) /
                                   2 - center[:, 0].unsqueeze(-1).repeat(1, 4)).clamp(min=0)
    _pt[:, :, 1] = _pt[:, :, 1] - ((lenH[:, np.newaxis].repeat(1, 4) - 1) /
                                   2 - center[:, 1].unsqueeze(-1).repeat(1, 4)).clamp(min=0)

    new_point = torch.zeros(pt.size())
    new_point[:, :, 0] = _pt[:, :, 0] + ul[:, 0].unsqueeze(-1).repeat(1, 4)
    new_point[:, :, 1] = _pt[:, :, 1] + ul[:, 1].unsqueeze(-1).repeat(1, 4)
    return new_point
